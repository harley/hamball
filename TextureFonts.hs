...
printFontsAt :: Vertex3 GLfloat -> (Maybe TextureObject,DisplayList)->  Int-> String -> IO()
printFontsAt (Vertex3 x y z) (fontTex,DisplayList _) st string =
   unsafePreservingMatrix $ do
      loadIdentity
      texture Texture2D $= Enabled
      textureBinding Texture2D $= fontTex
      ...
      rotate (90.0::GLfloat) (Vector3 1.0 0.0 0.0)
      let lists = map (toDisplayList (128*(fromIntegral st))) string
      alphaFunc $= Just (Greater,0.1:: Float)
      mapM_ callList lists --(map DisplayList [17..(32:: GLuint)])
      alphaFunc $= Nothing
