...
data Vec3d = Vec3d !(Float, Float, Float)
    deriving (Show, Eq)

scale :: Vec3d -> Float -> Vec3d
scale (Vec3d (ax,ay,az)) s = Vec3d (ax*s,ay*s,az*s)

inverseScale :: Vec3d -> Float -> Vec3d
...
vector3 :: Vec3d -> Vector3 Float
vector3 (Vec3d(a,b,c)) = Vector3 a b c

vertex3 :: Vec3d -> Vertex3 Float
vertex3 (Vec3d(a,b,c)) = Vertex3 a b c

vec3d :: Vector3 Float -> Vec3d
...
instance VectorSpace Vec3d Float where
