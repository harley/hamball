...
reactimate :: IO a
	      ...
	      -> (Bool -> b -> IO Bool)
              -> SF a b
	      -> IO ()
reactimate init sense actuate (SF {sfTF = tf0}) = undefined
{-    do
        a0 <- init
        let (sf, b0) = tf0 a0
        loop sf a0 b0
...
reactInternal rh fa bl =
  do rs@(ReactState {rsActuate = actuate,
     ...
     t' <- getCurrentTime
     let a' = fa a
         dt = diffUTCTime t' t -- I believe this is not limited to a day, ie dt is always > 0
         (sf',b') = (sfTF' sf) (fromRational $ toRational dt) a'
